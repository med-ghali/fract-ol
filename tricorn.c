...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/15 12:54:10 by mtagemou          #+#    #+#             */
/*   Updated: 2022/11/15 12:54:11 by mtagemou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

int	calc_stability_tricorn(t_complexe_num *point, int max_iteration,
int color_cycle)
{
	int		i;
	double	reel;
...
void	tricorn(t_vars *data)
{
	...
	while (++plan.x < MAX_WIDTH)
	{
		while (++plan.y < MAX_HEIGHT)
		{
			...
			color = calc_stability_tricorn(&point, data->max_iteration,
					data->color_cycle);
